...
public class OpenAiServiceImpl implements OpenAiService, org.springframework.beans.factory.InitializingBean {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        if (apiKey == null || apiKey.isEmpty() || apiKey.contains("your-openai-api-key")) {
            log.warn("[OpenAI] API key is empty or invalid. Falling back to the default key.");
            apiKey = "sk-Cc6rHMJ2Xv8IM6V5NARqysWmBVxyYQBtnD85bHSTUI8";
        }
        
        if (apiKey.length() > 10) {
            log.info("[OpenAI] Initialized. Using API key: {}...{} (length: {})", 
                apiKey.substring(0, 7), 
                apiKey.substring(apiKey.length() - 5),
                apiKey.length()
            );
        } else {
            log.warn("[OpenAI] Loaded API key is too short: '{}'", apiKey);
        }
    }
    ...
    public String transcribeAudio(String audioUrl) {
        ...
        try {
            ...
            // Wrap in ByteArrayResource with filename so RestTemplate uploads it properly
    ...
    private String callAiChatCompletions(String systemPrompt, String userPrompt) {
        try {
            ...
            Map<String, Object> response = openAiFeignClient.chatCompletions("Bearer " + apiKey, requestBody);
